...
8. 生命周期钩子
    ...
    >页面加载后就要展现的数据，可以在`created`、`beforeMount`、`mounted`请求（`vue-router`等封装了更方便的钩子，可以控制比如在数据加载完毕后再进行跳转）。
...
3. 响应式操作vue的data和vuex的store
    ...
    2. 无法检测对象/数组变动：
        ...
        >Vue不能检测到对象属性的添加或删除，所以添加`Vue.set/delete`。
